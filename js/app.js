...
(function(){
  ...
  var gN=document.getElementById('gNodes'),gR=document.getElementById('gRoutes');
  if(!gN||!gR)return;
  var pts=[
    [0,-182],[55,-165],[-55,-165],[110,-130],[-110,-130],[0,-125],
    [165,-72],[-165,-72],[82,-72],[-82,-72],
    [192,0],[-192,0],[148,45],[-148,45],
    [85,82],[-85,82],[0,145],[105,122],[-105,122],[0,192]
  ];
  var nodes=[];
  pts.forEach(function(p,i){
    var c=el('circle',{cx:p[0],cy:p[1],r:i%5===0?2.4:1.5,fill:i%7===0?'#F4F4EE':'#a8d916',opacity:0.65},gN);
    el('animate',{attributeName:'r',values:(i%5===0?'1.6;3.4;1.6':'0.9;2.2;0.9'),dur:(2.1+(i%6)*0.38)+'s',repeatCount:'indefinite'},c);
    nodes.push(p);
  });
  function pulse(){
    var a=nodes[Math.floor(Math.random()*nodes.length)];
    var b=nodes[Math.floor(Math.random()*nodes.length)];
    var mx=(a[0]+b[0])/2+(Math.random()-.5)*70,my=(a[1]+b[1])/2+(Math.random()-.5)*70;
    var d='M'+a[0]+' '+a[1]+' Q'+mx+' '+my+' '+b[0]+' '+b[1];
    var path=el('path',{d:d,fill:'none',stroke:'#a8d916','stroke-width':'0.9','stroke-dasharray':'0 400','stroke-linecap':'round',opacity:'0.6'},gR);
    el('animate',{attributeName:'stroke-dasharray',values:'0 400;55 345;0 400',dur:'2.2s',repeatCount:'1'},path);
    el('animate',{attributeName:'opacity',values:'0;0.6;0',dur:'2.2s',repeatCount:'1'},path);
    setTimeout(function(){try{path.remove();}catch(e){}},2300);
  }
  setInterval(pulse,1600);
  // Stagger first few pulses
